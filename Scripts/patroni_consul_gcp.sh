...
NODE_NAME=$(curl -s -H "Metadata-Flavor: Google" http://metadata.google.internal/computeMetadata/v1/instance/instance-id)
NODE_IP=$(curl -s -H "Metadata-Flavor: Google" http://metadata.google.internal/computeMetadata/v1/instance/local-ipv4)
...
cat <<EOF | sudo tee /etc/consul.d/consul.json
{
  ...
  "server": false,
  "bind_addr": "${NODE_IP}",
  "retry_join": ["provider=gce tag_key=Name tag_value=consul-server"]
}
EOF
